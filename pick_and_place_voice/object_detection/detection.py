...
class ObjectDetectionNode(Node):
    ...
    def _get_depth(self, x, y, window=5):
        """픽셀 주변 영역에서 유효한(0이 아닌) depth 값의 중앙값을 읽어옵니다."""
        frame = self._wait_for_valid_data(self.img_node.get_depth_frame, "depth frame")
        half = window // 2
        y0, y1 = max(0, y - half), min(frame.shape[0], y + half + 1)
        x0, x1 = max(0, x - half), min(frame.shape[1], x + half + 1)
        region = frame[y0:y1, x0:x1]
        valid = region[region > 0]
        if valid.size == 0:
            self.get_logger().warn(f"No valid depth near ({x},{y}).")
            return None
